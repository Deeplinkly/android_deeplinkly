package com.deeplinkly.android_deeplinkly.queue
...
import com.deeplinkly.android_deeplinkly.network.isTerminalHttp
...
object QueueProcessor {
     ...
     */
    fun startProcessing(apiKey: String) {
        // Process immediately
        processNow(apiKey)
    ...
    /**
     * Process pending deliveries to the attached listener
     */
    private suspend fun processDeliveryQueue() = withContext(Dispatchers.Main) {
        if (!SdkRuntime.hasListener()) {
            Logger.d("No listener attached, skipping delivery queue")
            return@withContext
        }

        // Anything already on its way to the listener is skipped rather than
        // sent a second time; deliverDeepLink releases the claim either way, so
        // a failed attempt is picked up again on the next pass.
        ...
        // Removal is deliverDeepLink's job, and it happens only once the
        // listener has taken the link.
        queue.forEach { pending -> SdkRuntime.deliverDeepLink(pending) }
    }

    /**
     * Process queues immediately (called when a listener attaches)
     */
    fun processNow(apiKey: String) {

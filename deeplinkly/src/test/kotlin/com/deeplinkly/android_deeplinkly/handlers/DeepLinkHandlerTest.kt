package com.deeplinkly.android_deeplinkly.handlers

import android.content.Context
import android.content.Intent
import androidx.test.core.app.ApplicationProvider
import com.deeplinkly.android_deeplinkly.core.SdkRuntime
import com.deeplinkly.android_deeplinkly.helpers.TestIntentBuilder
...
class DeepLinkHandlerTest {
    ...
    fun setUp() {
        ...
        com.deeplinkly.android_deeplinkly.core.DeeplinklyContext.app = context
        ...
        com.deeplinkly.android_deeplinkly.core.Prefs.of().edit().clear().commit()
    }
    ...
    fun tearDown() {
        ...
        com.deeplinkly.android_deeplinkly.queue.DeepLinkQueue.clearAll()
        com.deeplinkly.android_deeplinkly.core.Prefs.of().edit().clear().commit()
    }
    ...
    fun `handleIntent handles null intent gracefully`() {
        DeepLinkHandler.handleIntent(context, null, apiKey)
    ...
    /**
     * The no-listener case is the one that used to be expressed as "null
     * channel". It is the important one: a link that arrives before the host
     * has attached must survive rather than be dropped, because that is exactly
     * what a cold start looks like.
     */
    @Test
    fun `handleIntent retains the link when no listener is attached`() {
        SdkRuntime.clearListener()
        val intent = TestIntentBuilder.createClickIdIntent("no_listener_click")

        DeepLinkHandler.handleIntent(context, intent, apiKey)

        Thread.sleep(500)

        // Nothing can have been handed over, so the click must still be
        // recorded somewhere the processor will find it again.
        val queue = com.deeplinkly.android_deeplinkly.queue.DeepLinkQueue
        assertTrue(
            "a link that arrived with no listener attached must stay queued",
            queue.getResolveQueue().any { it.clickId == "no_listener_click" } ||
                queue.getDeliveryQueue().isNotEmpty()
        )
    }
    ...
    @Test
    fun `handleIntent queues deep link when no listener is attached`() {
        SdkRuntime.clearListener()
    ...
    fun `handleIntent ignores an intent relaunched from history`() {
        ...
        assertTrue(
            ...
            com.deeplinkly.android_deeplinkly.queue.DeepLinkQueue.getResolveQueue()
